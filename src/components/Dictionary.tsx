...
// fallback for headwords with no kosha card yet.
import samskrtamVerbLinks from '../dictionary/samskrtam-verb-links.tsv';
// dictionary + headword -> kosha per-lemma card token, generated by
// scripts/generate_kosha_links.py (H4843).
import koshaLinks from '../dictionary/kosha-links.tsv';
...
const koshaCardTokens: Map<string, string> = new Map(
  parseDictionaryTsv(koshaLinks)
    .filter((row) => row.status === 'ok')
    .map((row) => [`${row.dictionary}::${row.headword}`, row.card_token]),
);
...
function glossaryLinkUrl(dictionaryName: string, headword: string): string | null {
  const token = koshaCardTokens.get(`${dictionaryName}::${headword}`);
  if (token) {
    return `https://gasyoun.github.io/kosha/w/${token}.html`;
  }
  return samskrtamUrl(dictionaryName, headword);
}
...
const Dictionary: React.FC<DictionaryProps> = ({
  ...
  const renderSanskritValue = (value: string, key: string) => {
    ...
    const url = glossaryLinkUrl(name, value);
    const isKosha = url?.startsWith('https://gasyoun.github.io/kosha/');
    const label = (
      <>
        <span
      ...
      <span key={key}>
        {url ? (
          <a
            ...
            title={`${value} — ${isKosha ? 'kosha' : 'samskrtam.ru'}`}
          >
            {label}
          </a>
        ) : (
